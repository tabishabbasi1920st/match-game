...
class MatchGame extends Component {
  state = {
    activeTab: 'FRUIT',
    displayedImage: imagesList[0].imageUrl,
    score: 0,
    seconds: 60,
    gameOver: false,
    isDisplayRender: true,
    isThumbnailRender: true,
    isTabRender: true,
    isTimerRunning: true,
  }
  ...
  generateRandom(min = 0, max = imagesList.length) {
  ...
  onClickDifferentTabs = tabId => {
    this.setState({activeTab: tabId})
  }

  getRandomImage = () => {
    const randomImageUrl = imagesList[this.generateRandom()].imageUrl
    return randomImageUrl
  }

  onClickDifferentThumbnails = thumbnailId => {
    const {displayedImage} = this.state
    const currentDisplayedImgObj = imagesList.filter(eachObj => {
      return eachObj.imageUrl === displayedImage
    })
    const currentDisplayedImageId = currentDisplayedImgObj[0].id
    if (thumbnailId === currentDisplayedImageId) {
      this.setState(prevState => ({
        displayedImage: this.getRandomImage(),
      ...
      this.setState({
        gameOver: true,
        isTabRender: false,
        isDisplayRender: false,
        isThumbnailRender: false,
      })
    }
  }
  ...
  render() {
    ...
    const filteredList = imagesList.filter(eachObject => {
      return eachObject.category === activeTab
    })
          ...
          {isTabRender && (
            <div className="tab-container">
              {tabsList.map(eachObj => (
                <Tabs
                  isActive={activeTab === eachObj.tabId}
                  onClickDifferentTabs={this.onClickDifferentTabs}
                  key={eachObj.tabId}
                  eachObj={eachObj}
                />
              ))}
            ...
            <ul className="thumbnail-container">
              {filteredList.map(eachObj => (
                <Thumbnail
                  key={eachObj.id}
                  eachObj={eachObj}
                  onClickDifferentThumbnails={this.onClickDifferentThumbnails}
                />
              ))}
            </ul>
          )}
